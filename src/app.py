...
from src.ui_components import fraud_gauge, hero, inject_css, metric_card, risk_badge, scorecard_gauge, section_title
...
with st.sidebar:
    ...
    if scenario != "Manual custom transaction":
        if st.button("Load selected scenario", width="content"):
...
with tab1:
    ...
    with left:
        st.plotly_chart(fraud_gauge(prediction.fraud_score), width="content")
    with right:
...
with tab2:
    ...
    with col_b:
        ...
        fig.update_layout(height=430, yaxis={"categoryorder": "total ascending"}, paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(255,255,255,0.02)", font_color="#F9FAFB")
...
with tab3:
    ...
    st.dataframe(show_checks, width="content", hide_index=True)
    ...
    with col2:
        ...
        fig.update_layout(height=360, xaxis_range=[0, 105], yaxis={"categoryorder": "total ascending"}, paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(255,255,255,0.02)", font_color="#F9FAFB")
...
with tab4:
    ...
    with c1:
        ...
        fig.update_layout(height=370, paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(255,255,255,0.02)", font_color="#F9FAFB")
        st.plotly_chart(fig, use_container_width=True)
    with c2:
...
with tab7:
    ...
    with m1:
        ...
        fig.update_layout(height=360, paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(255,255,255,0.02)", font_color="#F9FAFB")
        st.plotly_chart(fig, use_container_width=True)
    with m2:
